...
#define S0(x) \
  (_mm256_xor_si256(ROR(x, 2), _mm256_xor_si256(ROR(x, 13), ROR(x, 22))))
#define S1(x) \
  (_mm256_xor_si256(ROR(x, 6), _mm256_xor_si256(ROR(x, 11), ROR(x, 25))))
#define s0(x) \
  (_mm256_xor_si256(ROR(x, 7), _mm256_xor_si256(ROR(x, 18), SHR(x, 3))))
#define s1(x) \
  (_mm256_xor_si256(ROR(x, 17), _mm256_xor_si256(ROR(x, 19), SHR(x, 10))))
...
#define Round(a, b, c, d, e, f, g, h, Kt, Wt)                                 \
  {                                                                           \
    __m256i T1 = _mm256_add_epi32(                                            \
        h,                                                                    \
        _mm256_add_epi32(                                                     \
            S1(e), _mm256_add_epi32(Ch(e, f, g), _mm256_add_epi32(Kt, Wt)))); \
...
inline void Transform(__m256i* state, const uint8_t* data[8]) {
  ...
  for (int t = 0; t < 16; t += 4) {
    for (int i = 0; i < 4; ++i) {
      ...
      W[t + i] = _mm256_setr_epi32(wt[0], wt[1], wt[2], wt[3], wt[4], wt[5],
                                   wt[6], wt[7]);
    }
  }
...
void sha256avx2_8B(const uint8_t* data0, const uint8_t* data1,
                   const uint8_t* data2, const uint8_t* data3,
                   const uint8_t* data4, const uint8_t* data5,
                   const uint8_t* data6, const uint8_t* data7,
                   unsigned char* hash0, unsigned char* hash1,
                   unsigned char* hash2, unsigned char* hash3,
                   unsigned char* hash4, unsigned char* hash5,
                   unsigned char* hash6, unsigned char* hash7) {
  ALIGN32 __m256i state[8];
  const uint8_t* data[8] = {data0, data1, data2, data3,
                            data4, data5, data6, data7};
  unsigned char* hashArray[8] = {hash0, hash1, hash2, hash3,
